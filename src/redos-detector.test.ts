  ...
  describe('isSafe', () => {
    describe('cases', () => {
      ...
      const cases: (Case | CaseWithFlags)[] = [
        ...
        [/^a+a+^a$/, false],
        [/^a+a+(^|)a$/, false],
        [/^(a{252,253}){1,}$/, false, new Set(['ignoreSnapshot'])],
        [/^(a{252,253}){1,2}$/, false, new Set(['ignoreSnapshot'])],
        [/^(a{2,3}){1,}$/, false],
        [/^((aa)|(aaa)){1,3}$/, false],
        [/^((aa)|(aaa)){1,}$/, false],
        [/^(a{1000,}b)+(a{999,}c)+$/, false, new Set(['ignoreSnapshot'])],
        [/^(a{1,2}a{1,2})*$/, false],
        [/^(?:(?:c|(a))){2}ac\1?a?$/, false],
        [/^(?:(?:c|(a))){2}ca\1?a?$/, false],
        // abababba
      ...
      cases.forEach(([regex, expectNoBacktracks, flags = new Set()]) => {
        ...
        it(source, () => {
          ...
          if (flags.has('missingAnchor')) {
            expect(neededDowngrade).toBe(true);
            expect(() =>
